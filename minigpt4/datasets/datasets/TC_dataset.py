...
class TCDataset(VQADataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        ...
        # --- TARGETED OVERSAMPLING START ---
        # 1. Identify positive (tumor) and negative (no tumor) samples
        tumor_annotations = [ann for ann in exist_annotation if ann["answer"].lower() == "yes"]
        healthy_annotations = [ann for ann in exist_annotation if ann["answer"].lower() == "no"]
        
        # 2. If tumor samples are fewer, duplicate them (3x oversampling)
        oversampling_factor = 2 
        for _ in range(oversampling_factor):
            self.annotation.extend(tumor_annotations)
        # 3. Shuffle the final list so copies aren't consecutive
        random.shuffle(self.annotation) 
        # --- TARGETED OVERSAMPLING END ---
        
        print(f"Final dataset size after oversampling: {len(self.annotation)}")
